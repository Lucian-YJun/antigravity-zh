...
2. **首字符倒排索引匹配 (性能优化)**
   - 全部 96 条正则规则均使用首字符锚定。引擎初始化时按首字符将规则分拣入桶，文本匹配时直接按首字寻址。
...
4. **958 项精细词条与规则（862 条精确词条 + 96 条动态正则，持续增补）**
   - 精细打磨的专属中文词典，覆盖日常交互、系统设置、Agent 执行状态与动态工作耗时。
...
│   ├── cockpit-zh.json            # 动态词典与正则规则 (862 词条 + 96 正则)
...
```bash
npm install        # 首次使用
npm run test:main  # 主程序注入测试（7 项）
npm run test:page  # 页面翻译器与倒排索引集成测试（16 项，需 jsdom）
```

测试集共计 23 项，验证通过率 100%。
