...
2. **首字符倒排索引匹配 (性能优化)**
   - 全部 131 条正则规则均使用首字符锚定。引擎初始化时按首字符将规则分拣入桶，文本匹配时直接按首字寻址。
...
4. **1,061 项精细词条与规则（930 条精确词条 + 131 条动态正则，持续增补）**
...
│   ├── cockpit-zh.json            # 动态词典与正则规则 (930 词条 + 131 正则)
...
```bash
npm install        # 首次使用
npm test           # 一键运行全部自动化测试（51 项）
npm run test:dict  # 词典与原生替换规则静态体检（13 项）
npm run test:main  # 主程序注入与原生菜单测试（7 项）
npm run test:page  # 页面渲染、同帧防闪烁与安全边界集成测试（31 项，需 jsdom）
```

测试集共计 51 项，验证通过率 100%。
